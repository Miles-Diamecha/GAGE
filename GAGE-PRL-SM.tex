...
The complete, executable workflow is archived at Zenodo (\texttt{GAGE\_repo v1.0.0}, DOI \href{https://doi.org/10.5281/zenodo.17537647}{10.5281/zenodo.17537647}). Section~S9 lists filenames, versions, and SHA-256 checksums; code listings are intentionally omitted here.

\noindent From the repository:
\begin{itemize}\setlength\itemsep{2pt}
  \item \texttt{pins/} including \texttt{pins.json} and \texttt{keq.json} (MS at $\mu=\MZ$); $\hat\alpha_2$ computed from $\hat\alpha/\sin^2\!\hat\theta_W$
  \item One-command rebuild: \texttt{bash build.sh} (Linux/macOS) or \texttt{.\textbackslash build\_win.bat} (Windows)
  \item Optional metrics: \texttt{scripts/metric\_eigs.py} → \texttt{outputs/metric\_results.json} ($K_{\rm eq}$ eigs/evecs, $\|\chi\|_{K_{\rm eq}}$, $\Lambda_\chi$, alignment)
  \item Deterministic artifacts with SHA-256 recorded in \texttt{checksums/SHA256SUMS.txt}
\end{itemize}
...
\noindent All pins, scripts, and figure data are archived at Zenodo (\texttt{GAGE\_repo v1.0.0}, DOI \href{https://doi.org/10.5281/zenodo.17537647}{10.5281/zenodo.17537647}); rebuild instructions and checksums are to follow.
